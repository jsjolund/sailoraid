...
$7.2921158 \times 10^{-5}$ $rad/s$\cite{nonlinear}. Using the projection matrix equation \eqref{Cosine_1} its possible to project $\bar{\omega}_ {ie}^i$ onto the $n$-frame
\begin{align}
...
Since the INS has to work in discrete time domain, equations \eqref{Eq.r_n_t}, \eqref{Eq.v_n_t} and \eqref{Eq.C_b_n_t} has to be discretized. The sampling time should be seen as a changing variable since it may not be constant instead it will fluctuate around $100Hz$, this means that $\Delta t_k = \Delta k_{k-1} - t_{k}$, this to improve the accuracy. The discrete dynamics are \cite{non-linear}, Using equations \eqref{Eq.r_n_t}, \eqref{Eq.v_n_t} and \eqref{Eq.C_b_n_t}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{velocity.eps}
